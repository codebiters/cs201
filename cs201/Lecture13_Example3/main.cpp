...
int main(int argc, char** argv)
{
    ...
    // Read the actual number of employes in the company
    ...
    // Calculate net salaries of the employees and store into the array
    cout << "\n\n Calculating the net salaries ...";
    calcNetSal(sal, numEmps);

    // Find the unlucky employees
    cout << "\n\n Loading the unlucky employees ...";
...
void calcNetSal(double sal[][2], int numEmps)
{
    for(int i = 0; i < numEmps; i++) { // Note this numEmps is local to this function
        if(sal[i][0] >= 0 && sal[i][0] <= 5000) {
            // There will be no text deduction
            sal[i][1] = sal[i][0];
        }

        else if(sal[i][0] >= 5001 && sal[i][0] <= 10000) {
            // Tax deduction is 5%
            sal[i][0] = sal[i][0] - (.05 * sal[i][0]);
        } else if(sal[i][0] >= 10001 && sal[i][0] <= 20000) {
            // Tax deduction is 10%
            sal[i][0] = sal[i][0] - (.10 * sal[i][0]);
        } else if(sal[i][0] >= 20001) {
            // Tax deduction is 15%
            sal[i][0] = sal[i][0] - (.15 * sal[i][0]);
        } else {
...
void findUnluckies(double sal[][2], int numEmps, int lucky[])
{
    for(int i = 0; i < numEmps; i++) { // Note that numEmps is local to this function
        if(sal[i][0] >= 0 && sal[i][0] <= 5000) {
            // No need to check for unlucky employees for this tax bracket
        } else if(sal[i][0] >= 5001 && sal[i][0] <= 10000) {
            markIfUnlucky(sal, numEmps, lucky, 5000, i);
        } else if(sal[i][0] >= 10001 && sal[i][0] < 20000) {
...
void markIfUnlucky(double sal[][2], int numEmps, int lucky[], int upperBound, int empNbr)
{
    for(int i = 0; i < numEmps; i++) {
         ...
         * unlucky even if the employee in the higher tax bracket is getting
         * the same amount of net salary as that of a person in the
         * lower tax bracket*/
